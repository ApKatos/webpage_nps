    ...
    <div>
      ...
      >
        <div class="graph" :id="'input-graph-' + variable.index">
          ...
          <HistogramC2
            :labelsInp="variable.labels"
            :dataInp="variable.data"
            :dataName="variable.varname"
            @update:value="variable.value = $event"
          ...
          <div v-if="variable.categ">
            <BinaryButtons
              ...
              v-on:update:value="variable.value = $event"
              :value="variable.value"
              :thrs="variable.thrs"
            >
            </BinaryButtons>
          ...
          <div v-else>
            <MySlider
              :label95q-lower="variable.alert_low"
              :label95q-upper="variable.alert_high"
              :maxim="variable.max"
              :minim="variable.min"
              :relevant-thresholds="variable.thrs"
              :categorial="variable.categ"
              :value="variable.value"
              v-on:update:value="variable.value = $event"
            >
            </MySlider>
      ...
      <div class="endbutton">
        <v-btn
          @click="callModel()"
          :disabled="!allVarsSubmitted"
          :loading="pressedButton"
        >
          SUBMIT
        </v-btn>
...
export default {
  ...
  data() {
    return {
      ...
      resultJSON: "",
      variablesinfo:
        "Color of the parameter informs you about the prevalence of chosen value in population this model was trained on.\nRed - unfilled value\nOrange - Very scarcely observed value\nGreen - average value\nPressing the variable name will navigate you to input value for the field.",
    };
  },
  methods: {
    loadData() {
